...
// ====== DRAW FUNCTION ======
function drawMeter(ctx, value, maxValue, label, step, colorFn) {
    ...
    const color = colorFn(value / maxValue);
...
function animate() {
    ...
    // Speed color → dari cyan ke biru terang
    const speedColor = (ratio) => `hsl(${190 - ratio * 60}, 100%, ${40 + ratio * 30}%)`;

    // RPM color → dari hijau → oranye → merah
    const rpmColor = (ratio) => {
        const hue = 120 - ratio * 120;
        return `hsl(${hue}, 100%, 50%)`;
    };

    drawMeter(sctx, currentSpeed, 240, "KMH", 20, speedColor);
    drawMeter(rctx, currentRPM, 8000, "RPM", 1000, rpmColor);
