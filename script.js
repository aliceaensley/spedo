...
async function searchYoutube(query) {
    ...
    try {
        ...
        if (data.items && data.items.length > 0) {
            ...
            elements.youtubeResults.classList.remove('hidden');
            // elements.youtubeResults.scrollLeft = 0; // Tidak perlu untuk vertikal scroll
...
function toggleYoutubeSearchUI(show) {
    // Sembunyikan atau tampilkan hanya results overlay
    if (elements.youtubeResults) {
        elements.youtubeResults.classList.toggle('hidden', !show);
    }
    // Search bar container tetap ada, kita hanya mengosongkan results
...
function toggleYoutubeUI(state) {
    ...
    if (state) {
        ...
        // Mulai dengan menampilkan search UI dan mengosongkan iframe
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    const handleSearch = () => {
        ...
        if (query.trim() !== '') {
            searchYoutube(query);
            // Kosongkan iframe saat memulai pencarian baru
            if (elements.browserIframe) elements.browserIframe.src = 'about:blank';
        }
    };
    ...
    // 7. Toggle Speedometer Mode
    if (elements.modeToggleIcon) {
        elements.modeToggleIcon.addEventListener('click', toggleSpeedometerMode);
    }
    
    // 8. Inisialisasi status awal
    setSpeedMode(1); // Default ke MPH
    setEngine(false); // Mesin mati
    setHeadlights(0); // Lampu mati
    setSeatbelts(false); // Seatbelt belum terpasang
    
    // 9. Mulai Pembaruan Data Vital (Health, Fuel)
    startVitalUpdates();
    
    // 10. Listener Kunci Esc untuk menutup YouTube
    document.addEventListener('keydown', (event) => {
        if (event.key === 'Escape' && isYoutubeOpen) {
            toggleYoutubeUI(false);
            // Pindahkan fokus kembali ke body agar ESC tidak mengganggu input lain
